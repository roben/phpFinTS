...
class MT940
{
    ...
    public function parse(string $rawData): array
    {
        ...
        // this is a speciality from some banks (Sparkasse), they devide groups of transactions (days)
        // we don't need that or care about it.
        // sadly, without changing the way this parser works, this may also remove SVWZ lines consisting only of a "-"
        // but then this line did not contain much information anyways...
        $rawData = str_replace($divider . '-' . $divider, $divider, $rawData);

        $soaDate = null;
        $segments = explode($divider . ':', $rawData);
        for ($i = 0, $cnt = count($segments); $i < $cnt; ++$i) {
            if (preg_match("/\+\@[0-9]+\@$/", trim($segments[$i]))) {
                $booked = false;
            }

            // handle start balance
            // 60F:C160401EUR1234,56
            if (preg_match('/^60(F|M):/', $segments[$i])) {
                // remove 60(F|M): for better parsing
                $segments[$i] = substr($segments[$i], 4);
                $soaDate = $this->getDate(substr($segments[$i], 1, 6));
                ...
                if (!isset($result[$soaDate])) {
                    ...
                    $amount = str_replace(',', '.', substr($segments[$i], 10));
                    $result[$soaDate]['start_balance']['amount'] = $amount;
                }
            } elseif (
                // found transaction
                // trx:61:1603310331DR637,39N033NONREF
                str_starts_with($segments[$i], '61:')
                && isset($segments[$i + 1])
                && str_starts_with($segments[$i + 1], '86:')
            ) {
                $transaction = substr($segments[$i], 3);
                $description = substr($segments[$i + 1], 3);
                ...
                preg_match('/^62F:/', $segments[$i]) // handle end balance
            ) {
                // remove 62F: for better parsing
                $segment = substr($segments[$i], 4);
                // this is always just a single line, ignore follow ups like `-'HNSHA:8:2+1574206''HNHBS:9:1+3'`
                $segment = explode($divider, $segment)[0];

                $soaDate = $this->getDate(substr($segment, 1, 6));
                if (isset($result[$soaDate])) {
                    $amount = str_replace(',', '.', substr($segment, 10, -1));
                    $cdMark = substr($segment, 0, 1);
                    if ($cdMark == 'C') {
